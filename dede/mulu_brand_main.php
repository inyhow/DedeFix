...
if($dopost=='saveall')
{
    ...
    foreach($aids as $aid)
    {
        ...
        if(!empty(${'isdel_'.$aid}))
        {
            $dsql->ExecuteNoneQuery("DELETE FROM `#@__brand_mulu` WHERE aid='$aid'");
            continue;
        }
        ...
        if($staold!=$sta)
        {
            $query1 = "UPDATE `#@__brand_mulu` SET sta='$sta',rpurl='$rpurl' WHERE aid='$aid' ";
        ...
        if($rpurl!=$rpurlold)
        {
            $query1 = "UPDATE `#@__brand_mulu` SET rpurl='$rpurl' WHERE aid='$aid' ";
            $dsql->ExecuteNoneQuery($query1);
        }
    }
...
else if($dopost=='add')
{
    ...
    $row = $dsql->GetOne("SELECT * FROM `#@__brand_mulu` WHERE keyword LIKE '$keyword'");
    if(is_array($row))
    {
        ShowMsg("关键字已存在库中！","-1");
        exit();
    }
    $inquery = "INSERT INTO `#@__brand_mulu`(keyword,rank,sta,rpurl) VALUES ('$keyword','$rank','1','$rpurl');";
...
elseif ( $dopost == 'fileKeywords') {
    ...
    if ( is_uploaded_file( $filename ) ) 
    {
        ...
        $dsql->SetQuery( "SELECT `keyword` FROM `#@__brand_mulu` " );//将SQL查询语句格式化
        ...
        foreach ( $keyArr as $k => $v ) 
        {
          ...
          // $rank   =   trim($v[2], ' ');
			 $rank   =   1;
            $sql = "INSERT INTO `#@__brand_mulu`  ( keyword, rank, sta, rpurl ) VALUES  ( '{$key}', {$rank}, 1, '{$rpurl}')";
...
{
   $dsql->ExecuteNoneQuery('TRUNCATE TABLE `#@__brand_mulu`') ? ShowMsg("你已成功清除所有的关键词！！", $ENV_GOBACK_URL ) : ShowMsg("清除关键词失败",-1) ;
...
$sql = "SELECT * FROM `#@__brand_mulu` $addquery ORDER BY rank DESC";
$dlist = new DataListCP();
$dlist->pageSize = 20;
$dlist->SetParameter("keyword",$keyword);
$dlist->SetTemplate(DEDEADMIN."/templets/mulu_brand_main.htm");
$dlist->SetSource($sql);
$dlist->Display();
