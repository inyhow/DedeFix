...
class DedeTagParse
{
    ...
    function SaveTo($filename)
    {
        ...
        //替换模板中的pageno变量 列表第二页开始加页码 2015/02/12
        global $lv;
        $PageNo = $lv->PageNo;
        $fieldsarr = $lv->Fields;
        $content = $this->GetResult();		
		$link_canonical= $fieldsarr['indexurl'].$fieldsarr['typedir'].'/';		
		$link_canonical= str_ireplace('{cmspath}','',$link_canonical);
		$link_canonical= str_ireplace('//','/',$link_canonical);
        if(isset($PageNo) && $PageNo == 1)
		{
        	$title = $fieldsarr['seotitle'];
        	$keywords = $fieldsarr['keywords'];
        	$description = $fieldsarr['description'];
        }else{
        	$title = $fieldsarr['typename'].'第'.$PageNo.'页'.'_'.$GLOBALS['cfg_webname'];
        	$keywords = $fieldsarr['typename'].'第'.$PageNo.'页'.','.$GLOBALS['cfg_webname'].','.$fieldsarr['keywords'];
        	$description = $fieldsarr['typename'].'第'.$PageNo.'页'.':'.$fieldsarr['description'];
        }		
		$content = str_ireplace('###title###', $title, $content);        
		$content = str_ireplace('###keywords###', $keywords, $content);        
		$content = str_ireplace('###description###', $description, $content);        
		$content = str_ireplace('###link_canonical###',$link_canonical, $content);
		// end    
